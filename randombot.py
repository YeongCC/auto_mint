...
def run_simulate_existing_user():
    async def simulate():
        conn = db_conn()
        cursor = conn.cursor()
        cursor.execute("""
            SELECT
                ...
                w.wallet_address
            FROM user_user u
            JOIN user_userprofile p ON u.id = p.user_id
            JOIN user_wallet w ON p.id = w.user_id
            WHERE u.username LIKE 'M@!_%%'
            ORDER BY RANDOM()
            LIMIT 1;
        """)
        row = cursor.fetchone()
        ...
        if not row:
            logging.warning("❌ There are no matching users（M@!_%）")
            return

        user_id, profile_id, wallet_id, wallet_address = row

        selected = random.choice(TOKENS)
        symbol = selected["symbol"]
        coin_id = selected["coin_id"]

        logging.info(f"🎯 Simulating existing user: {user_id}, token: {symbol} ({coin_id})")
        ...
        logging.info(f"✅ Done simulate existing user: {user_id}, TX: {tx_hash}")

    asyncio.run(simulate())


if __name__ == "__main__":
    run_simulate_existing_user()
